...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/POINT.conf are parsed in qt/POINT.cpp's main()
    ParseParameters(argc, argv);

    // Process help and version before taking care about datadir
    if (mapArgs.count("-?") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("Point Core Daemon") + " " + _("version") + " " + FormatFullVersion() + "\n";
    ...
    try {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "POINT:"))
                fCommandLine = true;
        ...
        if (fDaemon) {
            fprintf(stdout, "POINT server starting\n");
