...
This mechanism has been integrated into Bitcore-Node-Point which allows for notification to be broadcast through Insight API in one of two ways:
...
By default, The Point Core daemon will launch using the following constant:

```
static const int DEFAULT_SWIFTTX_DEPTH = 5;
```

This value can be overridden by passing the following argument to The Point Core daemon:
